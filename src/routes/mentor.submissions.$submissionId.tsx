...
import { SAMPLE_SUBMISSION, SUBMISSIONS } from "@/lib/mock-data";
...
function MentorDetail() {
  ...
  return (
    <AppShell role="mentor" title={sub.title} breadcrumbs={[{ label: "Mentor" }, { label: "Submissions", to: "/mentor/submissions" }, { label: sub.id }]}>
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <div className="space-y-6">
          <Card className="overflow-hidden p-0">
            ...
            <div className="grid gap-6 p-6 sm:grid-cols-2">
              {[
                ["Problem", SAMPLE_SUBMISSION.problem],
                ["Solution", SAMPLE_SUBMISSION.solution],
                ["Impact", SAMPLE_SUBMISSION.impact],
                ["Technical", SAMPLE_SUBMISSION.technical],
              ].map(([k, v]) => (
                <div key={k}>
          ...
          <Card className="p-6">
            ...
            <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
              <div className="rounded-md border border-border bg-muted/30 px-3 py-2">Demo: {SAMPLE_SUBMISSION.demoUrl}</div>
              <div className="rounded-md border border-border bg-muted/30 px-3 py-2">Deck: {SAMPLE_SUBMISSION.deckUrl}</div>
              <div className="rounded-md border border-border bg-muted/30 px-3 py-2">GitHub: {SAMPLE_SUBMISSION.githubUrl}</div>
              <div className="rounded-md border border-border bg-muted/30 px-3 py-2">Video: {SAMPLE_SUBMISSION.videoUrl}</div>
            </div>
          </Card>
        </div>
        ...
        <Card className="p-6">
          ...
          <div className="mt-5 grid gap-2">
            <Button variant="kafd" onClick={() => { toast.success("Approved for judging"); nav({ to: "/mentor/submissions" }); }}>
              <CheckCircle2 className="size-4" />Approve for judging
            </Button>
            <Button variant="outline" onClick={() => { toast.success("Returned for clarification"); nav({ to: "/mentor/submissions" }); }}>
